...
for domain_name, config in DOMAIN_CONFIGS.items():
    ...
    for page in range(1, 2):
        ...
        for card in job_cards:
            ...
            all_jobs.append((title, company, location, domaine, occupation, sector, job_type,  posted_date, job_url, skills_found, "Jobboom"))
...
df = pd.DataFrame(
    all_jobs,
    columns=[
        "titre", "entreprise", "region", "domaine", "metier", "sector", "specialite", "date_offre", "url", "competences", "source_site"
    ]
)
...
print(df.head())
df.to_csv('SCRAPPING/jobboom.csv', index=False, encoding='utf-8')
