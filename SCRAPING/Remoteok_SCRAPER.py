...
def scrape_jobs(driver, url, skills_list, domaine_label, max_jobs=10):
    driver.get(url)
    time.sleep(4)
    scroll_down(driver, pause_time=2, max_scrolls=2)
...
print(f"✅ Scraping terminé. {len(all_results)} offres sauvegardées dans 'OUMAYMA.csv'")
...
df.to_csv("SCRAPPING/remoteok.csv", index=False, encoding="utf-8")
